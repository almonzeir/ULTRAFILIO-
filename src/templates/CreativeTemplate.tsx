      ...
      `}</style>
            ...
            <section className="min-h-[90vh] grid lg:grid-cols-2">
                ...
                <div className="relative h-full bg-[hsl(var(--brand-2))] flex items-center justify-center p-12 overflow-hidden bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]">
                    ...
                    <motion.div
                        ...
                        className="relative z-10 w-80 h-80 md:w-[450px] md:h-[450px] neo-box bg-white rounded-[4rem] p-2 border-[8px] border-black overflow-hidden group shadow-[30px_30px_0px_0px_rgba(0,0,0,1)] transition-all duration-300"
                    >
            ...
            <section className="py-32 px-8 flex flex-col items-center border-y-4 border-black dark:border-white bg-white dark:bg-black">
                <motion.div initial={{ scale: 0.8, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} className="max-w-6xl text-center">
                    <h2 className="text-4xl md:text-7xl font-black uppercase italic leading-none mb-20 tracking-tighter">
                        "{about.extendedBio}"
                    </h2>
                    <div className="flex flex-wrap justify-center gap-6">
                        {about.skills?.map(skill => skill.tags?.map(tag => (
                            <motion.div key={tag} whileHover={{ y: -10, rotate: Math.random() * 10 - 5 }} className="neo-box px-8 py-4 bg-[hsl(var(--brand-2))] text-black font-black text-2xl uppercase italic">
                                {tag}
                            </motion.div>
                        )))}
                    </div>
            ...
            <section className="py-32 px-8 bg-black text-white">
                <div className="max-w-6xl mx-auto">
                    <div className="neo-box bg-yellow-400 text-black inline-block px-10 py-5 font-black text-4xl mb-24 rotate-[-2deg] uppercase">Journey_Log</div>
                    <div className="space-y-16">
                        {experience.map((job, i) => (
                            <motion.div key={i} whileHover={{ x: 20 }} className="neo-box p-12 bg-white dark:bg-black border-white flex flex-col md:flex-row justify-between items-start gap-12 group transition-all">
                                <div className="max-w-2xl">
                                    <h3 className="text-5xl font-black uppercase tracking-tighter mb-4 italic text-black dark:text-white group-hover:text-[hsl(var(--brand))]">{job.jobTitle}</h3>
                                    <div className="text-2xl font-black text-zinc-500 mb-8 tracking-widest uppercase">{job.company}</div>
                                    <ul className="space-y-4">
                                        {job.responsibilities.map((resp, ridx) => (
                                            <li key={ridx} className="flex gap-4 text-xl font-bold italic text-black dark:text-white/70">
                                                <Zap className="shrink-0 text-yellow-500 fill-current" />
                                                <span>{resp}</span>
                                            </li>
                                        ))}
                                    </ul>
                                </div>
                                <div className="neo-box bg-black text-white dark:bg-white dark:text-black px-8 py-4 font-black uppercase tracking-widest text-xl rotate-3">
                                    {job.dates}
                                </div>
                            </motion.div>
                        ))}
            ...
            <section id="projects" className="bg-black">
                <div className="grid md:grid-cols-2">
                    {projects.map((project, idx) => (
                        <motion.div key={idx} whileInView={{ opacity: 1 }} className="group relative h-[70vh] border-b-4 border-r-4 border-white overflow-hidden">
                            <div className="absolute inset-x-0 bottom-0 p-12 bg-black z-30 translate-y-full group-hover:translate-y-0 transition-transform duration-500 border-t-4 border-white">
                                <div className="flex justify-between items-start mb-6 text-white">
                                    <h3 className="text-5xl font-black uppercase tracking-tighter italic">{project.name}</h3>
                                    <div className="neo-box bg-[hsl(var(--brand))] p-4 rotate-12">
                                        <ArrowUpRight className="w-8 h-8 text-black" />
                                    </div>
                                </div>
                                <p className="text-xl font-bold text-white/70 max-w-xl mb-8 leading-tight italic">{project.description}</p>
            ...
            {((education && education.length > 0) || (certifications && certifications.length > 0)) && (
                <section className="py-32 px-8 bg-white dark:bg-[#050510] border-t-4 border-black dark:border-white">
                    <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-24">
                        {education && education.length > 0 && (
                            <div>
                                <h2 className="text-6xl font-black uppercase italic mb-16 tracking-tighter">Academic.</h2>
                                <div className="space-y-12">
                                    {education.map((edu, i) => (
                                        <div key={i} className="neo-box p-8 bg-white dark:bg-black rotate-[-1deg] hover:rotate-0 transition-all">
                                            <div className="flex items-center gap-4 mb-4">
                                                <GraduationCap className="w-10 h-10 text-[hsl(var(--brand))]" />
                                                <h4 className="text-3xl font-black uppercase italic">{edu.degree}</h4>
                                            </div>
                                            <div className="text-xl font-bold text-zinc-500 uppercase tracking-widest">{edu.institution}</div>
                                            <div className="mt-8 font-black text-zinc-400 font-mono tracking-widest uppercase">{edu.startDate} — {edu.endDate}</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="space-y-24">
                            {certifications && certifications.length > 0 && (
                                <div>
                                    <h2 className="text-6xl font-black uppercase italic mb-16 tracking-tighter">Accredited.</h2>
                                    <div className="flex flex-wrap gap-6">
                                        {certifications.map((cert, i) => (
                                            <div key={i} className="neo-box bg-purple-500 text-white px-8 py-4 font-black text-xl italic hover:scale-110 transition-transform">
                                                {cert}
                                            </div>
                                        ))}
                                    </div>
                            ...
                            {languages && languages.length > 0 && (
                                <div>
                                    <h2 className="text-6xl font-black uppercase italic mb-16 tracking-tighter">Bilingual.</h2>
                                    <div className="grid gap-8">
                                        {languages.map((lang, i) => (
                                            <div key={i} className="flex items-end gap-6 border-b-8 border-dotted border-black dark:border-white pb-4">
                                                <span className="text-5xl font-black italic">{lang.name}</span>
                                                <span className="text-xl font-bold text-zinc-500 uppercase italic opacity-60">[{lang.level || 'Expert'}]</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
